...
function export($type,$generaltasklist,$tasklist)
{
	...
	//设定头部	
	$Sheet->setCellValue("A1",date('Y')."年市政府".$tztype[$type]."推进落实清单")
				->mergeCells("A1:H1")
				->setCellValue("A2","序号")
				->mergeCells("A2:A3")
	...
	$name = 'ExportExcel/'.date('Y').date('mdHis').$tztype[$type].'台账'.'.xlsx';	
	$execlName=dirname(__FILE__).'\ExportExcel\\'.date('Y').date('mdHis').iconv('UTF-8','GB2312',$tztype[$type]).iconv('UTF-8','GB2312','台账').'.xlsx';	
