...
function getdata($type)
{
	...
	$sql="SELECT id,taskid,remark,isover FROM taskreview WHERE viewtime>'".$firstday."' ORDER BY id";
	$reviewlist=$link->getall($sql);
	...
	foreach($generaltasklist as $g)
		{
	//查询每个总体目标包含的工作目标		
	$sql="SELECT type,id,target,title,investment  FROM task WHERE generaltaskid='".$g["id"]."' AND type=".$type." ORDER BY id";
		...
		foreach($tlist as $t){
			...
			if(count($reviewlist)>0)
			{
				foreach($reviewlist as $review){
					if($review['taskid']==$t['id']){
					$t['remark']=$review['remark'];
					$t['isover']=$review['isover'];	
					}
				}
			}			
			$a=array(
...
{
	global $task_type;
	//$over=array("1"=>"未完成","2"=>"基本完成","3"=>"完成");	
	$over=array("1"=>"未完成","2"=>"完成");	
	...
	$Sheet->getColumnDimension("D")->setWidth(5.5);
	$Sheet->getColumnDimension("E")->setWidth(20);
	$Sheet->getColumnDimension("F")->setWidth(10);
	$Sheet->getColumnDimension("G")->setWidth(10);
	$Sheet->getColumnDimension("H")->setWidth(37.5);
	  ...
	  foreach($generaltasklist as $v)
	  {
		...
		if(count($tasklist)>0)
		{
			...
			{						
				...
				$firstrowJ="J".($startrow+1);
				$firstrowK="K".($startrow+1);
				if($t["li"]>1){
				...
				$endrowJ="J".($startrow+$t["li"]);
				$endrowK="K".($startrow+$t["li"]);
				$Sheet->mergeCells("$firstrowA:$endrowA")
					  ...
					  ->mergeCells("$firstrowJ:$endrowJ")
					  ->mergeCells("$firstrowK:$endrowK");					  
							  }
				$rowid++;
				$Sheet->setCellValue($firstrowA,$rowid)				  	 
					 ...
					 ->setCellValue($firstrowH,$t["remark"])
					 ->setCellValue($firstrowJ,$t["depts"])					
					 ->setCellValue($firstrowK,$t["id"]);
					 //设置自动换行及垂直居中
				if(!empty($t["isover"])) $Sheet->setCellValue($firstrowI,$over[$t["isover"]]);
				...
				$Sheet->getStyle($firstrowH)->getAlignment()->setWrapText(true)
															->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
				$Sheet->getStyle($firstrowI)->getAlignment()->setWrapText(true)
															->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);															
				$Sheet->getStyle($firstrowJ)->getAlignment()->setWrapText(true)
															->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
					 //输出进度	
