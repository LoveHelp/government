...
if(!empty($_REQUEST['do'])){
	...
	}else if($do == 'getHistoryReview'){
		$taskid = $_REQUEST['tid'];
		$res = getHistoryReview(0, $taskid);
		$res = json_encode($res);
	}else if($do == 'modRviewById'){
		$res = modRviewById($userid);
	}
	echo $res;
}
...
function getHistoryReview($pdo, $taskid){
	$sql = "select tr.id id, u.uid, u.uname, u.roleid, tr.viewtime, tr.progress, tr.remark, tr.viewtime, tr.backendtime, tr.isover state from taskreview tr join user u on tr.userid=u.uid where tr.taskid=:taskid order by tr.viewtime desc, tr.progress desc, id desc;";
	$param=array();
	$param[':taskid'] = $taskid;
...
function reviewTask($userid, $userRoleId){
	...
	//获取定期报类型
	$tmp = "";
	if(intval($res['backtype']) > 0 && intval($res['backtype']) < 7)
	...
	if(!empty($tmp)){
		...
		case 1:if($month % 3 == 1) $month = intval($month) + 2;										//季报
			...
			$backtime = date('Y').'-'.$month.'-'.$month_day[intval($month)];	
			break;
		case 2:$backtime = date('Y').'-'.$month.'-'.$month_day[intval($month)];						//月报
			break;
		...
		$param[':backendtime'] = $res['backtype'];	//按时报
...
function gettaskofpage($generaltaskid, $state, $type, $target, $page=0, $size=15, $deptid=0){
	...
	for($i=0; $i<count($data); $i++){
		for($j=0; $j<count($data[$i]['proj']); $j++){
			...
			//获取待审核的反馈记录条数
			$data[$i]['proj'][$j]['fb_no_rvw_cnt'] = 0;
			$data[$i]['proj'][$j]['fb_no_rvw_cnt'] = GetCountOfFbNoRvw($pdo, $tmp_taskid);
		}
	}	
	return $data;
}
...
function modRviewById($userid)
{
	...
	$sql = "update taskreview set userid=:uid, {$key}='{$value}', viewtime=now(), backendtime=:endtime where id=:id;";
	$pdo = new mysql;
	$param = array(
		':uid' => $userid,
		':endtime' => GetBackEndTime($pdo, $taskid),
		':id' => $rvwId
	);
	$res = $pdo->update2($sql, $param);
	if($res == -1)
		return 0;
	return 1;	
}

function GetBackEndTime($pdo, $taskid){
	//获取台账的报告类型：定期报、按时报
	$sql_2 = "select ifnull(onbacktime, regbacktype) backtype from task where id={$taskid};";
	$res = $pdo->getRow($sql_2);
	//定期报，默认：季报
	...
	if(!empty($tmp)){
		...
		return $res['backtype'];	//按时报
}

//根据台账获取需要审核的反馈记录条数
function GetCountOfFbNoRvw($pdo, $taskid)
{
	$sql= "select fb.id, fbr.rvwstate rvws from taskfeedback fb left join fb_review fbr on fb.id=fbr.fbid where fb.taskid={$taskid};";
	if(empty($pdo))
		$pdo = new mysql;
	$res = $pdo->getAll($sql);
	if(empty($res))
